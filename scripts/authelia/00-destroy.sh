#!/bin/bash
# 00-destroy.sh — Destroy existing Authelia LXC container
# Run from: workstation (proxima-centauri)
#
# Safe to run if CT-316 doesn't exist (exits 0).
# Clears SSH known_hosts entry for clean rebuild.
#
# IMPORTANT: ZFS datasets are NOT destroyed. This is intentional.
# The data (SQLite DB, config, users, secrets) persists across rebuilds.
# To truly wipe everything: zfs destroy -r pbs-zfs/services/authelia
#
set -euo pipefail
...
echo ""
echo "Next: bash scripts/authelia/01-create.sh"
